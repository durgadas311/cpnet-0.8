...
struct cpmfcb *get_dir_entry(DIR *dirp, struct cpmfcb *search_fcb, int first) {
  ...
  while (1) {
    ...
    if (!match_fcb(search_fcb, &last_search)) continue;
    
    if (stat(dp->d_name, &stbuf) != 0) {
      stbuf.st_size = 0;
      continue;   /* can't stat? - don't show it */
    }
    if (S_ISDIR(stbuf.st_mode)) continue;  /* don't show dirs either */
...
int delete_files(struct cpmfcb *fcb) {
  ...
  while (1) {
    ...
    if (!dp) break;  /* no more entries */

    setname(&lfcb, dp->d_name);
    if (!match_fcb(fcb, &lfcb)) continue;
    
    if (stat(dp->d_name, &stbuf) != 0) {
      stbuf.st_size = 0;
      continue;   /* can't stat? - don't delete it */
    }
    if (S_ISDIR(stbuf.st_mode)) continue;  /* don't delete dirs either */
